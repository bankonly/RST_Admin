...
const findById = async (model, { _id, select = "-password -refresh_token", throw_error = true }) => {
    _id = _.isArray(_id) ? _id : _id.toString();
    if (!_.isObjectId(_id)) throw new Error(`400::invalid ${_id || "objectId"} for ${model.modelName}`);
    ...
    if (throw_error) {
        if (!excute || (_.isArray(excute) && excute.length < 1)) throw new Error(`400::no record ${_id || "objectId"} for ${model.modelName}`);
    }
...
const remove = async (model, { _id, removeFromDb = false, deleted_at = Date.now(), ENP_APP = process.env.NODE_ENV }) => {
    ...
    if (removeFromDb || process.env.NODE_ENV === "development") {
...
const find = async (
    model,
    { throw_error = false, many = true, deleted_at = null, sort = { created_at: 1 }, paginate = { page: 1, limit: 20, useSliceFunc: false, data_set: false, force: false }, _id, condition = {}, populate, select, search = { key: [], key_word: null, options: "i" }, error_code = 204 }
) => {
    ...
    if (paginate.limit || paginate.page || paginate.data_set.limit || paginate.data_set.page || paginate.force) {
        if (paginate.data_set) {
            paginate.limit = parseInt(paginate.data_set.limit) || false;
            paginate.page = parseInt(paginate.data_set.page) || false;
        } else {
...
const findExist = async (model, { condition = {}, key, message = null }) => {
    condition = { deleted_at: null, ...condition };
    const excute = await model.findOne(condition);
    if (excute) {
        if (message) throw new Error(`400::${message}`);
        throw new Error(`400::${key} already exist`);
    }
};
